...
class ErrorBoundary extends Component {
  ...
  componentDidCatch(error, info) {
    console.error("[Kemet]", error, info);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ minHeight:"100vh", background:"#000", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:20, padding:24, fontFamily:"'Cairo',sans-serif", textAlign:"center" }}>
          ...
          <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"2rem", color:"#D4AF37", fontWeight:300 }}>حدث خطأ غير متوقع</div>
          <div style={{ color:"rgba(255,255,255,0.5)", fontSize:"0.85rem", maxWidth:500, lineHeight:1.7 }}>{this.state.message || "يرجى تحديث الصفحة"}</div>
          <button onClick={() => window.location.reload()} style={{ background:"linear-gradient(135deg,#D4AF37,#A08020)", border:"none", borderRadius:10, padding:"12px 32px", color:"#000", fontWeight:700, cursor:"pointer" }}>تحديث الصفحة</button>
        </div>
      );
    }
